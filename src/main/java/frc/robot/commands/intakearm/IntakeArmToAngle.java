...
public class IntakeArmToAngle extends CommandBase {
  ...
  public boolean isFinished() {
    double angleError = Math.abs(IntakeArmSub.getAngle()- angle);
    double velocityError = Math.abs(IntakeArmSub.getVelocity());
    //return true if arm is at desired angle and exitWhenReached is true
    return angleError < RobotSettings.INTAKE_ARM_ANGLE_ERROR
      && velocityError < RobotSettings.INTAKE_ARM_VELOCITY_ERROR
      && exitWhenReached;
  }
}
